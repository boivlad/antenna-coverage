...
describe('Test basic case', function() {
  ...
  it('1) common antennas count 3, coverage 595, antennas array [43, 2, 300, 4, 554, 10]',
    function() {
      ...
      const result = antennaCoverageCost(3, 595, [43, 2, 300, 4, 554, 10]);
  ...
  it('2) common antennas count 1, coverage 1, antennas array [1, 1, 1, 1]',
    function() {
      let start = performance.now()
      const result = antennaCoverageCost(1, 1, [1, 1, 1, 1]);
      let end = performance.now()
      sumTime += end - start
  ...
  it('3) common antennas count 2, coverage 50, antennas array [20, 0, 3, 1]',
    function() {
      let start = performance.now()
      const result = antennaCoverageCost(2, 50, [20, 0, 3, 1]);
      let end = performance.now()
      sumTime += end - start
  ...
  it(
    ...
    function() {
      let start = performance.now()
      const result = antennaCoverageCost(5, 240, [13, 0, 50, 25, 60, 5, 155, 70, 165, 70]);
      let end = performance.now()
      sumTime += end - start
